...
import { isValidCIN } from "@/lib/utils/validation";
...
export default function VerifyMCA() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    // Enhanced validation
    if (!isValidCIN(cin)) {
      setMsg("Please enter a valid 21-character CIN");
      return;
    }
  ...
  return (
    <Protected allowedRoles={["user"]}>
      <PageShell title="MCA Verification" subtitle="Fetch CIN/DIN details, status, charges, and directors">
          ...
          {verificationResult && (
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  <p className="text-xs text-neutral-500 mb-1">CIN</p>
                  <p className="text-sm font-medium text-neutral-900">{verificationResult.summary.cin}</p>
                </div>
                <div>
                ...
                <div>
                  <p className="text-xs text-neutral-500 mb-1">Directors</p>
                  <p className="text-sm font-medium text-neutral-900">{verificationResult.summary.keyFields.directorsCount}</p>
                </div>
              </div>
        ...
        <p className="text-sm mb-2 mt-6" style={{ fontFamily: 'var(--font-geist)', fontWeight: 600 }}>Recent MCA checks</p>
